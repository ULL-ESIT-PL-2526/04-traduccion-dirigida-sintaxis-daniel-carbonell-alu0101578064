...
describe('Parser Failing Tests', () => {
    ...
    test('should respect precedence and associativity with floats', () => {
        expect(parse("2.5 + 3.5 * 2.0")).toBe(9.5); // 2.5 + 7.0 = 9.5
        expect(parse("10.0 - 1.5 ** 2")).toBe(7.75); // 10.0 - 2.25 = 7.75
        expect(parse("5.5 * 2.0 / 2.2")).toBe(5); // (5.5 * 2.0) / 2.2 = 11.0 / 2.2 = 5
    });

    test('should handle parentheses with highest precedence', () => {
        expect(parse("(2 + 3) * 4")).toBe(20); // Frente al 14 sin paréntesis
        expect(parse("10 - (6 / 2)")).toBe(7);
        expect(parse("2 ** (3 ** 2)")).toBe(512);
        expect(parse("(2 ** 3) ** 2")).toBe(64); // Forzando asociatividad por la izquierda
        expect(parse("100 / (2 + 3) * 2")).toBe(40); // (100 / 5) * 2 = 20 * 2 = 40
    });
});
